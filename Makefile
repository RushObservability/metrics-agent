...
helm-lint:
	helm lint helm-chart --set rushRemoteWrite.enabled=true --set rushRemoteWrite.allowAnonymous=true --set 'rushRemoteWrite.url=$(RUSH_REMOTE_WRITE_URL)'

helm-template:
	helm template metrics-agent helm-chart --namespace monitoring --set rushRemoteWrite.enabled=true --set rushRemoteWrite.allowAnonymous=true --set 'rushRemoteWrite.url=$(RUSH_REMOTE_WRITE_URL)' >/dev/null
